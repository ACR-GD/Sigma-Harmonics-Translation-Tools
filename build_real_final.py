...
test_rom = "/Users/acr/Develop/sigma-harmonics/sigma_en_real_final.nds"
...
for e in file_entries:
    ...
    raw_path = os.path.join(mod_cpk_dir, filename)
    if os.path.exists(raw_path):
        # Determine if original was CRILAYLA (zero-padded)
        orig_data = orig_cpk[e.fileoffset:e.fileoffset+e.filesize]
        is_zero_padded = (orig_data[:8] == b'\x00' * 8)
        
        comp = None
        if is_zero_padded:
            zeros_count += 1
            # Must use the CRILAYLA compressed .cache file
            cache_files = glob.glob(os.path.join(mod_cpk_dir, f"{filename}_*.cache"))
            if cache_files:
                with open(cache_files[0], 'rb') as f:
                    comp = bytearray(f.read())
                # Strip CRILAYLA signature to match original
                if comp[:8] == b"CRILAYLA":
                    comp[:8] = b"\x00" * 8
        else:
            others_count += 1
            # Must use the RAW uncompressed/custom .bin file!
...
print(f"Processed {zeros_count} CRILAYLA files and {others_count} Non-CRILAYLA files.")
